...
const ShopScreen: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!resCat.ok || !resProd.ok) throw new Error('Lỗi khi tải dữ liệu từ server');
  ...
  const handleCate = async (categoryId: string) => {
    ...
    try {
      const res = await fetch(`${ipAddress}/api/products/category/${categoryId}`);
  ...
  const sections: Section[] = [
    ...
    { type: 'specialOffers', data: products.slice(0, 3), title: 'Ưu đãi đặc biệt' },
              ...
              renderItem={({ item: category }) => (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    selectedCategory === category._id && styles.categoryCardSelected,
                  ]}
                >
